...
**/
  ...
  public Instruction executeInstruction () {
    Instruction pc = getPC();
   /* if (pc instanceof InstanceInvocation) {
    	String className = ((InstanceInvocation) pc).getInvokedMethodClassName();
		String MethodName = ((InstanceInvocation) pc).getInvokedMethodName();
		System.out.println("ThreadInfo: Original "+className+"."+MethodName);
	}*/
    ...
    //System.out.println(pendingSUTExceptionRequest+" "+attributes+" "+ATTR_SKIP_INSN_EXEC);
    if ((pendingSUTExceptionRequest == null) && ((attributes & ATTR_SKIP_INSN_EXEC) == 0)){
        try {
         /* if (pc instanceof InstanceInvocation) {
        	String className = ((InstanceInvocation) pc).getInvokedMethodClassName();
  			String MethodName = ((InstanceInvocation) pc).getInvokedMethodName();
  			System.out.println("ThreadInfo: have we change it? "+className+"."+MethodName);
  			if (pc instanceof VirtualInvocation && "checkCallingOrSelfPermission(Ljava/lang/String;)I".equals(MethodName)) {
  				System.out.println("VirtualInvocation!");
